/**
 * Emoji name lookup for `EmojiPicker`'s search-by-name. Derived once from
 * unicode-emoji-json@0.9.0 ("data-by-emoji.json" for the name, ordered by
 * "data-ordered-emoji.json", Unicode's own presentation order), trimmed to
 * just [emoji, name] pairs. This file *is* the output — the package was never
 * added as a dependency, only used once to generate it. To pick up a newer
 * Unicode release, reinstall that package, re-run the same extraction (zip
 * its ordered list with its by-emoji map, keep only ".name") and replace the
 * array below.
 */
export const EMOJI_NAMES: readonly (readonly [emoji: string, name: string])[] = [
  ['😀', 'grinning face'],
